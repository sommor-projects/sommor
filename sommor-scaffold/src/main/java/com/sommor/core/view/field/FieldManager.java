...
public class FieldManager {
            ...
            for (Field field : clazz.getDeclaredFields()) {
                if (! Modifier.isStatic(field.getModifiers())) {
                    ...
                    if (null != getter && null != setter) {
                        ...
                        definition.setRender(getRenderMethod(clazz, field));
    ...
    private static Method getRenderMethod(Class clazz, Field field) {
        String fieldName = field.getName();
        String renderMethodName = "render" + fieldName.substring(0,1).toUpperCase() + fieldName.substring(1);
        Method method = null;
        try {
            method = clazz.getMethod(renderMethodName, new Class[] {FieldRenderContext.class});
            method.setAccessible(true);
        } catch (NoSuchMethodException e) {
            // ignore
        }
